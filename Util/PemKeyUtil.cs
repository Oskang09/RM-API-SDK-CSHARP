...
{
    // REFERENCE: https://stackoverflow.com/a/49889680
    public class PemKeyUtil
    {
        ...
        public static RSACryptoServiceProvider GetRSAProviderFromPemString(String pemString)
        {
            bool isPrivateKeyFile = true;
            if (pemString.StartsWith(pempubheader) && pemString.EndsWith(pempubfooter))
                isPrivateKeyFile = false;

            if (pemString.StartsWith(pemrsapubheader) && pemString.EndsWith(pemrsapubfooter))
                return DecodeX509PublicKey(DecodeOpenSSLRSAPublicKey(pemString));

            byte[] pemkey;
            if (isPrivateKeyFile)
                pemkey = DecodeOpenSSLPrivateKey(pemString);
            else
                pemkey = DecodeOpenSSLPublicKey(pemString);
        ...
        static byte[] DecodeOpenSSLRSAPublicKey(String instr)
        {
            const String pempubheader = "-----BEGIN RSA PUBLIC KEY-----";
            const String pempubfooter = "-----END RSA PUBLIC KEY-----";
